...
GET(
  url = "https://github.com/sportsdataverse/sportsdataverse-data/releases/download/espn_nba_pbp/play_by_play_2026.rds",
  write_disk("play_by_play_2026.rds", overwrite = TRUE),
  timeout(300)  # timeout di 5 minuti
)


# NBA season (2025-)2026
schedule <- load_nba_schedule(seasons = 2026)
...
pbp <- readRDS("play_by_play_2026.rds")
...
exit_g1  <- c(exit_g1$end_game_seconds_remaining)     |> sort(decreasing = TRUE)
...
enter_g2 <- c(2880, enter_g2$end_game_seconds_remaining) |> sort(decreasing = TRUE)
...
exit_g4  <- exit_g4$end_game_seconds_remaining      |> sort(decreasing = TRUE)
...
exit_g5  <- exit_g5$end_game_seconds_remaining     |> sort(decreasing = TRUE)
...
exit_g6  <- c(exit_g6$end_game_seconds_remaining,0)     |> sort(decreasing = TRUE)
...
#unione 6 gare -> contiene tutti i tiri contro minnesota con gobert in campo e fuori
...
plot_gobert <- shots_serie_totale_half %>%
  mutate(gobert_status = ifelse(gobert_on_court,
                                "Gobert in campo",
                                "Gobert in panchina"),
         gobert_status = factor(gobert_status,
                                levels = c("Gobert in campo", "Gobert in panchina"))
  ) %>%
  ggshotchart(
    x = "coordinate_x",
    y = "coordinate_y",
  ...
  ggtitle("Tiri tentati da Denver nella serie contro Minnesota nel 2026") +
...
shots_serie_totale <- shots_serie_totale %>%
  mutate(home_game = if_else(origine %in% c("gara1", "gara2", "gara5"), TRUE, FALSE))

#variabile scarto tra squadre
shots_serie_totale <- shots_serie_totale %>%
  mutate(scarto = case_when(
    origine %in% c("gara1", "gara2", "gara5") ~ home_score - away_score,
...
ggplot(grafico_prob, aes(x = seconds/60, y = prob, 
  ...
  labs(
    ...
    y = "Probabilità (%)",
    title = "Probabilità di tiro dal midrange, serie playoff 2026",
...
#home game true e gobert in campo 
exp(-1.0264)
#0.3583
0.3583/(1+0.3583)
#26,38%
...
1- (619.12/628.93)
#0.015 -> capacità limitata di spiegare la variabilità